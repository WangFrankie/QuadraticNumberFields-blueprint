...
{uses "zsqrtd_ideal_membership"}[Those concrete ideals] have quotient
descriptions by $`\mathbb{Z}/p\mathbb{Z}`, which supply the primality proofs.
:::
...
{uses "zsqrtd_quotients"}[The quotient descriptions] prove that both ideals are
prime in the relevant concrete splitting branch.
:::
...
in quadratic number fields, building on
...
For odd primes not dividing the parameter,
...
and $`3`, serving as concrete tests of
