...
The discriminant of the $`\mathbb{Z}[\sqrt d]` basis
(`discr_zsqrtd_basis`). The {uses "zsqrtd_order"}[$`\mathbb{Z}[\sqrt d]`]
basis has discriminant $`4d`. This is the basis calculation for the
non-$`1 \pmod 4` branch.
:::
...
The discriminant of the half-integral basis
(`discr_zOnePlusSqrtOverTwo_basis`). The
{uses "zomega_order"}[$`\mathbb{Z}[(1+\sqrt d)/2]`] basis has discriminant
$`d` when $`d = 1 + 4k`.
:::
...
The quadratic-field discriminant formula (`discr_formula`). For squarefree
$`d \ne 1`, the discriminant of $`\mathbb{Q}(\sqrt d)` is $`d` in the
$`1 \pmod 4` branch and $`4d` otherwise, combining
...
Transport of the discriminant formula to abstract quadratic fields
...
Dedekind-domain criterion for the project order $`\mathbb{Z}[\sqrt d]`
(`isDedekindDomain_iff_mod_four_ne_one`). The project model
{uses "zsqrtd_order"}[$`\mathbb{Z}[\sqrt d]`] is Dedekind exactly in the same
branch where
...
Concrete discriminant examples (`discr_gaussian`, `discr_eisenstein`,
`discr_Qsqrtd_neg_five`). The library includes checks for the Gaussian,
Eisenstein, and $`\mathbb{Q}(\sqrt{-5})` examples, exercising
